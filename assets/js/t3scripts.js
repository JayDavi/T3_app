...
function report(){
  ...
  var income = document.getElementById("income").value;
  var cost = totalCost();
  document.getElementById("theIncome").innerHTML = 'Your total healthcare cost is:  &nbsp;&nbsp; $' +
  cost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + '<br>Premiums Above Medicare Cost: &nbsp;&nbsp; $' +
  section1().toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + '<br>Long Term Care will cost you:  &nbsp;&nbsp; $' +
  section2().toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + '<br>The amount not covered by medicare is:  &nbsp;&nbsp; $' +
  section3().toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
  '<br><br>' + 'To Live Comfortably Save... <br>$' +
  byYear.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + ' per year<br>$' +
  byMonth.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + ' per month<br>$' +
  byWeek.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + ' per week<br>$' +
  byDay.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + ' per day<br>';
}
